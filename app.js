...
let notes = [];
 ...
 // ---- HELPERS ----
async function save() {
  // 1. Tetap simpan ke LocalStorage (sebagai cadangan di browser ini)
  localStorage.setItem('kanban_tasks', JSON.stringify(tasks));
  localStorage.setItem('kanban_notes', JSON.stringify(notes));

  // 2. KIRIM KE FIREBASE (Agar sinkron antar perangkat)
  if (typeof db !== 'undefined') {
    try {
      console.log("Sedang menyinkronkan ke Cloud...");
      await db.collection("learning_data").doc("user_1").set({
        tasks: tasks,
        notes: notes,
        lastUpdated: Date.now()
      });
      console.log("✓ Data berhasil tersimpan di Firestore!");
    } catch (error) {
      console.error("Gagal simpan ke Cloud. Cek tab Rules di Firebase!", error);
    }
  } else {
    console.error("Variabel 'db' tidak ditemukan! Cek urutan script di index.html.");
  }
}
